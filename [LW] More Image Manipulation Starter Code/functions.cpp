...
void removeHorizontalLine(unsigned int index, Pixel image[MAX_WIDTH][MAX_HEIGHT], int width, int height) {
  ...
  for(unsigned int i = index; i < height2; ++i){
...
/*void removeVerticalLine(unsigned int index, Pixel image[MAX_WIDTH][MAX_HEIGHT], int width, int height) {
  ...
  unsigned int height2 = height;
  Pixel** image2 = createImage2(MAX_WIDTH, MAX_HEIGHT);
  

  //transfer over again
  for(unsigned int i = 0; i < width2; ++i){
    for (unsigned int j = 0; j < height2; ++j){
      image2[i][j].r = image[i][j].r;
      image2[i][j].g = image[i][j].g;
      image2[i][j].b = image[i][j].b;
    }
  }
  for(unsigned int i = index; i < width2; ++i){
    for (unsigned int j = 0; j < height2; ++j){
        image2[i][j].r = image2[i+1][j].r;
        image2[i][j].g = image2[i+1][j].g;
        image2[i][j].b = image2[i+1][j].b;
    }
  }
  //clear the original image[][] array
  //transfer over again
  for(unsigned int i = 0; i < width2; ++i){
    for (unsigned int j = 0; j < height2; ++j){
      image[i][j].r = image2[i][j].r;
      image[i][j].g = image2[i][j].g;
      image[i][j].b = image2[i][j].b;
    }
  }
}*/
...
int removeVerticalLines(Pixel image[MAX_WIDTH][MAX_HEIGHT], int width, int height, unsigned int frequency) {
  int column = 1; // start at 1 since first row will not be removed
  unsigned int skippedLines = 1; // already skipped the first line
  if (frequency == 0){
    return width;
  }
  while (column < width) {
    if (skippedLines == frequency) { // remove this line
      removeVerticalLine(column, image, width, height);
      width--;
      skippedLines = 0;
    }
    else {
